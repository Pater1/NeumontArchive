...
public class GraphEdge : MonoBehaviour {
    ...
    void Update () {
        ...
        if(Mathf.Abs(dist - distance) > 0.25f * (1.0f - AnnealingFactor)){
            ...
            a.GetComponent<Rigidbody2D>().AddForce((a.transform.position - transform.position + ((Vector3)Random.insideUnitCircle * AnnealingFactor)).normalized * delta * Mathf.Lerp(annealingScalar, 1.0f, AnnealingFactor));
            b.GetComponent<Rigidbody2D>().AddForce((b.transform.position - transform.position + ((Vector3)Random.insideUnitCircle * AnnealingFactor)).normalized * delta * Mathf.Lerp(annealingScalar, 1.0f, AnnealingFactor));
        }
